...
public class SenseClient {
	...
	private static HttpClient	client	= com.almende.util.ApacheHttpClient
												.get();
	...
	public static void login(AccountBean account) {
		...
		try {
			...
			if (webResp.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
				...
				LOG.info("Logged into Sense:" + account.getEmail() + " : "
						+ result);
			}
	 ...
	 */
	public static String getSensorValue(String sensorId) {
		HttpGet httpGet = null;
		String base = Settings.SenseBaseUrl;
		httpGet = new HttpGet(URI.create(base + "/sensors/" + sensorId
				+ "/data?last=true"));
		HttpResponse webResp;
		String result=null;;
		try {
			webResp = client.execute(httpGet);
			result = EntityUtils.toString(webResp.getEntity());
			if (webResp.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
				LOG.warning("Received HTTP Error Status:"
						+ webResp.getStatusLine().getStatusCode() + ":"
						+ webResp.getStatusLine().getReasonPhrase());
	...
	public static void createUser(AccountBean account) {
		...
		try {
			...
			if (webResp.getStatusLine().getStatusCode() != HttpStatus.SC_CREATED) {
				...
				ObjectNode res = (ObjectNode) JOM.getInstance()
						.readTree(result);
